...
class WxPayController extends BaseController
{
    ...
    public function getOpenId(Request $request,$id)//获取用户openid
    {
        ...
        if (!$code) {
            $url = urlencode(config('home.WEB').'/wxpay/getOpenId/'.$id);
            $sendUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . config('home.APPID') . '&redirect_uri=' . $url . '&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect';
            return redirect($sendUrl);
        }
        $tokenUrl = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' . config('home.APPID') . '&secret=' . config('home.SECRET') . '&code=' . $code . '&grant_type=authorization_code';
...
//            }
            if($openid) {
                ...
                $data['notify_url'] = config('home.NOTIFY_URL');
                $data['openid'] = $openid;
                ...
                $parametersArr = array();
                $parametersArr['appId'] = config('home.APPID');
                ...
                return view('home.wxPay.wxChatPay', ['jsApiParameters'=>$jsApiParameters]);
            }else{
