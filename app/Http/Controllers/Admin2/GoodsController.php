...
use App\Http\Requests\Admin\GoodsRequest2 as GoodsRequest;
...
class GoodsController extends Controller
{
    ...
    {
        $flag=$mark;
        return view('admin.goods2.index',compact('flag'));
    }
    ...
    public function addGoods(GoodsRequest $request)//执行添加商品操作
    {
        ...
        if ($res) {
            return redirect(url('admin2/goods/index',array('mark'=>1)))->with('success','添加商品成功');
        } else {
    ...
    public function actGoodsArea(GoodsAreaRequest $request)//执行添加专区商品操作
    {
        ...
        if ($res) {
            return redirect(url('admin2/goods/index',array('mark'=>2)))->with('success','添加商品成功');
        } else {
            return back()->with('error','添加商品失败');
        }
    }
    ...
    {
        $date=$this->goods->where(function($query) use ($request){
            if($request->has('name'))
        ...
        return view('admin.goods2.goodsList',compact('date','res','goodsClass'));
    }
    ...
    public function goodsAreaList(Request $request)//加载专区商品列表
    {
        ...
        return view('admin.goods2.goodsAreaList',compact('date','res'));
    }

    public function goodsEdit($id)//加载商城商品修改视图
    {
        $res=Goods::where(['id'=>$id])->first();
        return view('admin.goods2.goodsEdit',compact('id','res','data'));
    }
    ...
    public function goodsAreaEdit($id)//加载专区商品修改视图
    {
        $res=Goods::select(['id','name','title','pic','storage','sale','type','factory_code'])
            ->where(['id'=>$id])->first();
        return view('admin.goods2.goodsAreaEdit',['res'=>$res,'id'=>$id]);
    }
    ...
    public function smallPicEdit($id,$flag)//加载修改商品轮播图的视图
    {
        ...
        return view('admin.goods2.smallPicEdit',compact('id','data','flag'));
    }
