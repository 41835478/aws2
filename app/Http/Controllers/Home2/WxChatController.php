...
class WxChatController extends BaseController
{
    ...
    public function loginRegister(Request $request)
    {
        ...
        if (!$code) {
            $url = urlencode(config('home2.WEB').'/wxChat/loginRegister');
            $sendUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . config('home2.APPID') . '&redirect_uri=' . $url . '&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect';
