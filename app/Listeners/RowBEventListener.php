...
class RowBEventListener implements ShouldQueue
{
    /**
...
//                        $this->row->index(Redis::lpop('rowb'),5);
//                    }
//                }
            }
        }
    ...
    {
        if($num<=$aim_level){
            $prev_id=Rowb::where(['id'=>$rowId])->value('prev_id');
            if($prev_id){
                $num++;
    ...
    public function getSelfPrevInfo($now_row_id,$aim_row_id,$order_id)
    {
        ...
        $to_row=Rowb::where(['id'=>$aim_row_id])->first(['id','user_id','current_level','current_generate','order_id']);
    ...
    public function updateDynasty($from_row,$to_row,$order_id)//升级循环判断
    {
        ...
        $where=array('row_id'=>$to_row->id,'current_level'=>$to_row->current_level,'user_id'=>$to_row->user_id,'flag'=>2,'status'=>1);
        if($to_row->current_generate<=2){
            ...
            if($main){
                ...
                $res1=$this->promoteInfo($from_row->id,$to_row->id,$to_row->current_level,2,$feeConfig['deductFee'],$to_row->user_id,$promoteInfo,1);
                if($res1){
                    ...
                    if($res2->promote_fee>=$feeConfig['promoteFee']){
                        ...
                        if($up_row_id){
                            ...
                            $res4=$this->promoteInfo($to_row->id,$up_row_id,$to_row->current_level,2,$feeConfig['promoteFee'],$to_row->user_id,$promoteInfo2,2);
                            if($res4){
            ...
            if($main){
                ...
                if($res1){
                    ...
                    if($res2->promote_fee>=$feeConfig['promoteFee']){
                        ...
                        if(($to_row->current_level + 1)<10){
                            $this->setPromoteRecode($to_row->id, 2, $to_row->user_id, $to_row->current_level + 1, $promote_fee);
                        }
        ...
        if($to_row->current_generate>=4&&$to_row->current_level==10){
            $where1=array('row_id'=>$to_row->id,'current_generate'=>$to_row->current_generate,'type'=>2,'user_id'=>$to_row->user_id);
    ...
    public function feeConfig($dynasty,$level)//$dynasty 第几代 通过当前代数得到要向上级交的升级费，上级的到的见点奖，上级的每个点的见点扣的费用
    {
        ...
        if($dynasty<=3){
            ...
            if($level==3){
                ...
                $date['redFee']=56.66;//round(510/9,2)
            }
            if(in_array($level,[4,5,6])){
                $date['promoteFee']=6000;
                $date['deductFee']=1851.86;
                $date['redFee']=1148.14;
            }
            if(in_array($level,[7,8,9,10])){
