...
<div class="wrapper wrapper-content animated fadeInRight">
    <div class="row">
        <div class="col-sm-12">
         ...
         <form action="{{url('admin2/order/index')}}" method="get">
                        ...
                        <tbody>
                            ...
                            <tr class="gradeX">
                                ...
                                @if ($v->status == 1) <a style="color:#F00">无法操作</a>
                                    @elseif ($v->status == 2) 
                                       <a href="{{url('admin2/order/edit',['id'=>$v->id])}}" style="color:#00F">发货</a>|
                                    @elseif ($v->status != 1 && $v->status != 2) 

                                    @endif
                                    
                                    <a href="{{url('admin2/order/orderinfo',['id'=>$v->id])}}">详情</a>  <!-- |
...
<script type="text/javascript">
    $(function(){
        ...
        function displayAjax(data)
        {
            ...
            $.ajax({
                'url':'{{url("admin2/goodsClass/whetherDisplay")}}',
                'data':data,
                'async':true,
                'type':'post',
                'dataType':'json',
        ...
        $('#topLevel').change(function(){
            ...
            if(id!=''){
                ...
                $.ajax({
                    'url':'{{url("admin2/goodsClass/secondClass")}}',
                    'data':{'id':id},
                    'async':true,
                    'type':'post',
                    'dataType':'json',
