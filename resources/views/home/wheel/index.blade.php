...
<div class="draw-bg">
    <p class="draw-hint">今日有<b class="draw-hint_num" id="draw-hint_num">{{$lucky}}</b>次抽奖机会</p>
    <div class="draw-show">
...
<div class="draw_kuang" style="display: none;">
    <img src="{{asset('home/images/fly_1.png')}}" alt="">
...
<div class="draw_yh" style="display: none;">
    <img src="{{asset('home/images/fly_3.png')}}" alt="">
    <p>什么都没有抽到明天继续加油</p>
    <input type="button" value="确定" class="draw_sure">
</div>
<script type="text/javascript">
    ...
    var draw = new drawAll({
        ...
        odds: [{{$mod->angel_1}},{{$mod->angel_2}},{{$mod->angel_3}},{{$mod->angel_4}},{{$mod->angel_5}},{{$mod->angel_6}}],//奖品对应的抽奖几率
        doEle: document.getElementById('draw-change'),//旋转元素
         inChange: '{{$key}}'//内定选中元素，可不填，不填则根据几率抽奖
    });

    var timer = '{{$timer}}';//0、可以抽奖 1、不可以抽奖
    if(timer==1){
        $("#draw-hint_num").text("0");
        $(".draw-btn").css("background-image","url('{{asset('home/images/fly_5.png')}}')");
    }else{
        document.getElementById('draw-btn').addEventListener('click', function () {//点击事件
            $.ajax({
                'url':"{{url('wheel/luckyStart')}}",
                'asnyc':true,
                'type':'post',
                'dataType':'json',
                success:function(res){
                    if(res.status){
                        draw.inChange=res.data.key;
                        ...
                        setTimeout(function () {
                            ...
                            if(draw.num <= 0){
                                $(".draw-btn").css("background-image","url('{{asset('home/images/fly_2.png')}}')")
                            }
                            if(index == 3){
                                $(".draw_yh").show();
                            }else{
